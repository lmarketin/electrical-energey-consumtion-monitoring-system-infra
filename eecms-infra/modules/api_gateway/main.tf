...
resource "aws_apigatewayv2_integration" "apigw_integration" {
  ...
  integration_uri    = var.lb_listener_arn//aws_lb_listener.listener.arn

  connection_id      = aws_apigatewayv2_vpc_link.vpclink_apigw_to_alb.id
  connection_type    = "VPC_LINK"
  description        = "VPC integration"

  depends_on      = [aws_apigatewayv2_vpc_link.vpclink_apigw_to_alb, aws_apigatewayv2_api.api_gw]//TODO  aws_lb_listener.listener
 }
...
resource "aws_apigatewayv2_authorizer" "lambda_auth" {
  ...
  authorizer_result_ttl_in_seconds = 0  # Disable caching TODO
}
